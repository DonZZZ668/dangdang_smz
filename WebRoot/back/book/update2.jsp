...
<%@ page import="com.baizhi.entity.Book"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
		...
		<form action="${pageContext.request.contextPath}/book/updateOneBook" method="post" enctype="multipart/form-data">
			<table>
				<tr>
					...
					<input type="hidden" name="book.createDate" value="<fmt:formatDate value='${book.createDate }' pattern='yyyy-MM-dd'/>"/>
					...
					<td>
						<select name="book.categoryId" class="el-select__inner inner2">
							<c:forEach var="c" items="${requestScope.clist }">
								<option value="${c.id }" <c:if test="${c.id==book.categoryId }">selected="selected"</c:if>>${c.name }</option>
							</c:forEach>
						</select>
					</td>
					<td>字数：</td>
				...
				<tr>
					<td>出版时间：</td>
					<td><input type="date" name="book.pressDate" class="el-input__inner" value="<fmt:formatDate pattern='yyyy-MM-dd' value='${book.pressDate}'/>"></td>
					<td>内容简介：</td>
				...
				<tr>
					<td>印刷时间：</td>
					<td><input type="date" name="book.printDate" class="el-input__inner" value="<fmt:formatDate pattern='yyyy-MM-dd' value='${book.pressDate}'/>"></td>
					<td>作者简介：</td>
