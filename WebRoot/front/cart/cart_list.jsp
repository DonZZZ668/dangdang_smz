					...
					<table class="tabl_buy" id="tbCartItemsNormal">
						...
						<c:forEach var="c" items="${sessionScope.cart }">
							<tr class='td_no_bord'>
								...
								<td>
									<a href="${pageContext.request.contextPath}/book/showDetail?book.id=${id}">${c.value.book.name }</a>
								</td>
								<td class="buy_td_5">
								...
								<td >
									<input id="${c.value.book.id }" class="del_num" type="number" size="3" maxlength="4" />
									<a href="#"  onclick="updateCart('${c.value.book.id}')">变更</a>
								</td>
								<td>
									<a href="${pageContext.request.contextPath}/cart/deleteOneItem?id=${c.value.book.id}">删除</a>
								</td>
							</tr>
						</c:forEach>
					</table>
